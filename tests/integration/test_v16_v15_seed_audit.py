...
from morocco_elections.v16.audit import audit_v15_seed
...
from morocco_elections.v16.demography import load_hcp_rgph2014_arrondissement_identifiers, load_hcp_rgph2014_territorial_universe
from morocco_elections.v16.demography import load_hcp_rgph2014_individuals
from morocco_elections.v16.evidence import derive_contest_legal_regime_links, validate_official_source_registry
from morocco_elections.v16.publication import PublicationContext, _bind_contract_tables, _semantic_facts, _verify_demographic_source_rows, sha256_file
...
def test_hcp_official_arrondissement_codes_match_all_v15_comm2015_units() -> None:
    workbook = ROOT / "data/raw/elections/v16/demography/hcp_rgph2014_population_legale_12_regions.xlsx"
    ...
    seed = json.loads((ROOT / "metadata/v16/comm2015_council_candidates.seed.json").read_text(encoding="utf-8"))
    sources = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))["sources"]
...
def test_readiness_audit_reports_external_territorial_coverage_without_claiming_ready() -> None:
    ...
    process = subprocess.run(
        [sys.executable, str(ROOT / "tools/audit_v16_readiness.py"), "--summary"],
        cwd=ROOT, capture_output=True, text=True, check=False,
    )
    assert process.returncode == 0  # V16 parent relations resolve the historical semantic violations.
...
def test_development_build_materializes_verified_legal_evidence(tmp_path: Path) -> None:
    ...
    context = PublicationContext(
        release_id="V16-DEV-AUDIT", as_of_date="2026-09-15",
