...
from morocco_elections.v16.history import affiliation_at_date, geographies_comparable, validate_geo_history, validate_party_history
from morocco_elections.v16.institutional import reproduce_largest_remainder_allocation, validate_candidacies_and_seats
...
from morocco_elections.v16.schema import ddl
from morocco_elections.v16.validation import validate_legal_regimes, validate_rows
...
def test_direct_boundary_comparison_requires_matching_package_geometries(tmp_path: Path) -> None:
    ...
    def context() -> PublicationContext:
        return PublicationContext(
            release_id="V16", as_of_date="2021-09-08", package_root=tmp_path,
            files=[
...
def _file(sha256: str = "0" * 64) -> dict:
    return {"release_id": "V16", "relative_path": "data.duckdb", "sha256": sha256, "source_id": "S", "acquired_at": "2026-09-14", "license_status": "REDISTRIBUTABLE", "claim_class": "OBSERVED_FACT", "fact_status": "OBSERVED", "privacy_review_required": True}


def _matrix(scope_id: str = "ACQUIRED") -> dict:
    return {"release_id": "V16", "scope_id": scope_id, "universe_ids_json": json.dumps([f"U-{scope_id}"], separators=(",", ":")), "acquired": 1, "expected": 1, "covered": 1, "missing": 0, "non_comparable": 0, "redistribution_forbidden": 0, "status": "COMPLETE"}
...
def _publication_context(package_root: Path) -> PublicationContext:
    ...
    try:
        ...
        connection.execute("INSERT INTO warehouse_metadata VALUES ('V16', 16, 'V15', '2026-09-14')")
    finally:
        connection.close()
    ...
    for build_id, report_name in (("B1", "clean-build-1.json"), ("B2", "clean-build-2.json")):
        clean_report = json.dumps({
            ...
            "commands": [
                ...
                {"name": "validate_v16", "command": "python tools/validate_v16.py", "exit_code": 0},
            ],
        }, sort_keys=True)
    ...
    context = PublicationContext(
        ...
        datasets={
            ...
            "legal_decisions": [],
            "warehouse_metadata": [{"release": "V16", "schema_version": 16, "source_release": "V15", "as_of_date": "2026-09-14"}],
