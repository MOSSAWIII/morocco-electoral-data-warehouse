...
from morocco_elections.v16.bo6374 import validate_visual_candidate
...
def test_page6105_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    ...
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6105_visual_transcription.candidate.json").read_text(encoding="utf-8"))
...
def test_page6106_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6106_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6107_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6107_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6108_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6108_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6109_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6109_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6110_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6110_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6111_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6111_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6112_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6112_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6113_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6113_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6114_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6114_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6115_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6115_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6116_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6116_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6117_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6117_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
...
def test_page6118_visual_transcription_is_source_pinned_and_candidate_only() -> None:
    registry = json.loads((ROOT / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8"))
    source = next(row for row in registry["sources"] if row["source_id"] == "MA_SGG_BO_6374_DECREE_2_15_402_COMMUNES_2015")
    payload = json.loads((ROOT / "metadata/v16/bo6374_page6118_visual_transcription.candidate.json").read_text(encoding="utf-8"))
    assert validate_visual_candidate(payload, source) == []
