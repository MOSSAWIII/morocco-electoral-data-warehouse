...
from morocco_elections.warehouse.portability import (
    SourceEvidenceError,
...
def test_reproducible_acquisition_rejects_changed_remote_bytes(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("morocco_elections.warehouse.portability.urllib.request.urlopen", lambda *_args, **_kwargs: Response())
...
def test_reproducible_acquisition_rejects_unavailable_remote(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("morocco_elections.warehouse.portability.urllib.request.urlopen", unavailable)
