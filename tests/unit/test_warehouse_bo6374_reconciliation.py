...
def test_bo_tafra_reconciliation_is_complete_bijective_and_fail_closed() -> None:
    ...
    assert report["candidate_rows"] == 1307
    assert report["identity_matches"] == 1301
    ...
    assert report["explained_differences"] == 1284
    assert report["exact_name_matches"] == 1152
    assert report["fuzzy_unique_name_matches"] == 149
    matched_ids = [row["tafra_commune_id"] for row in payload["rows"] if row["tafra_commune_id"] is not None]
    assert len(matched_ids) == len(set(matched_ids)) == 1301
