...
def test_bo_tafra_reconciliation_is_complete_bijective_and_fail_closed() -> None:
    ...
    assert report["candidate_rows"] == 987
    assert report["identity_matches"] == 981
    ...
    assert report["explained_differences"] == 966
    assert report["exact_name_matches"] == 846
    ...
    assert len(matched_ids) == len(set(matched_ids)) == 981
