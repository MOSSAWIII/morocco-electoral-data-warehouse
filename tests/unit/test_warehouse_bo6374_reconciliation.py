...
def test_bo_tafra_reconciliation_is_complete_bijective_and_fail_closed() -> None:
    ...
    assert report["candidate_rows"] == 1163
    assert report["identity_matches"] == 1157
    ...
    assert report["explained_differences"] == 1141
    assert report["exact_name_matches"] == 1020
    ...
    assert len(matched_ids) == len(set(matched_ids)) == 1157
