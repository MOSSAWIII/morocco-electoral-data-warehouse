...
from morocco_elections.warehouse.package import (
    BUNDLE_NAME,
    CATALOG_NAME,
    CONTRACT_NAME,
    DATABASE_NAME,
...
def _make_package(root: Path) -> str:
    ...
    try:
        ...
        connection.close()
    _write_json(root / CONTRACT_NAME, {"release": "TEST-SNAPSHOT"})
    ...
    context = PublicationContext(
        release_id="test-snapshot", as_of_date="2026-09-21", files=manifest["files"],
