...
def configure_paths(data_dir: str | Path | None = None) -> None:
    ...
    global ROOT, V8, V9, RAW, FILES
    ROOT = paths.project_root
    V8 = paths.v8_workbook
    V9 = paths.v9_workbook
    RAW = paths.data_root / "raw"
    FILES = {
        "COMM2015": paths.comm2015,
        "COMM2021": paths.comm2021,
        "COUNCIL2021": paths.council2021,
        "MPS": paths.parliamentary_members,
        "HCP2024": paths.hcp_population_2024,
    }
...
def main(data_dir: str | Path | None = None):
    ...
    for ws in wb.worksheets:
        if ws.title in {"RAW_HCP_RGPH2024_FULL"}:
            continue
        ws.sheet_view.showGridLines = True
    V9.parent.mkdir(parents=True, exist_ok=True)
    wb.save(V9)
    print(f"Saved {V9}")
