...
def _download(row: Mapping[str, Any], destination: Path) -> None:
    ...
    request = urllib.request.Request(str(row["source_url"]), headers={"User-Agent": "morocco-elections-v16/1"})
    try:
...
def portable_evidence_root(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="v16-source-evidence-") as temporary:
        root = Path(temporary)
