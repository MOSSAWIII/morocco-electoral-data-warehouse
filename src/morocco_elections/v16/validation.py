...
from morocco_elections.v16.contracts import FIELD_VOCABULARIES, TABLE_CONTRACTS, VOCABULARIES
...
class V16ValidationError(ValueError):
...
def validate_rows(table: str, rows: Iterable[Mapping[str, Any]]) -> list[ValidationIssue]:
    ...
    if table not in TABLE_CONTRACTS:
        return [ValidationIssue("UNKNOWN_TABLE", table, "<contract>", "table is absent from the V16 contract")]
...
def validate_or_raise(issues: Sequence[ValidationIssue]) -> None:
    if issues:
        raise V16ValidationError(issues)
