...
HCP_PATH = Path("data/raw/elections/v16/demography/hcp_rgph2014_population_legale_12_regions.xlsx")
DEFAULT_ARTIFACT_PATH = Path("data/exports/open/v16-development/bo6374_tafra_2015_reconciliation.json")
DEFAULT_REPORT_PATH = Path("data/exports/open/v16-development/bo6374_tafra_2015_reconciliation_report.json")
...
def _load_candidates(root: Path, bo_source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for path in sorted((root / "metadata/v16").glob("bo6374_page*_visual_transcription.candidate.json")):
    ...
    if len(rows) != 713:
        raise ValueError(f"expected 713 BO candidate rows, observed {len(rows)}")
    return rows
...
def build_reconciliation(root: Path) -> dict[str, Any]:
    ...
    parent_seed = json.loads((root / "metadata/v16/comm2015_geo_parents.seed.json").read_text(encoding="utf-8"))
    ...
    for index, candidate in enumerate(candidates):
        ...
        if index not in matched:
            ...
            if group is None:
                raise ValueError(f"unmatched large-city council has no verified V16 parent: {candidate['candidate_id']}")
            rows.append({
                **base,
                ...
                "v16_communal_parent_id": group["parent_geo_id"],
    ...
    report = {
        ...
        "unmatched_with_verified_v16_parent": sum(
    ...
    return {"schema_version": 1, "scope": "BO6374_PAGES_6105_6118_CANDIDATE_ONLY", "sources": descriptors, "rows": rows, "report": report}
...
def validate_reconciliation(payload: Mapping[str, Any], root: Path) -> list[ValidationIssue]:
    ...
    try:
        ...
        return [ValidationIssue("BO_TAFRA_RECONCILIATION_INPUT_INVALID", "bo_tafra_reconciliation", "V16", str(exc))]
    if payload.get("schema_version") != expected["schema_version"] or payload.get("scope") != expected["scope"]:
        issues.append(ValidationIssue("BO_TAFRA_RECONCILIATION_HEADER_MUTATION", "bo_tafra_reconciliation", "V16", "schema_version or scope differs"))
    if payload.get("sources") != expected["sources"]:
        issues.append(ValidationIssue("BO_TAFRA_RECONCILIATION_SOURCE_MUTATION", "bo_tafra_reconciliation", "V16", "source fingerprints or identifiers differ"))
    ...
    if set(actual_rows) != set(expected_rows):
        issues.append(ValidationIssue("BO_TAFRA_RECONCILIATION_ROW_SET_MUTATION", "bo_tafra_reconciliation", "V16", "candidate row identifiers differ"))
    ...
    if payload.get("report") != expected["report"]:
        issues.append(ValidationIssue("BO_TAFRA_RECONCILIATION_REPORT_MUTATION", "bo_tafra_reconciliation", "V16", "report differs from recomputation"))
    return issues
