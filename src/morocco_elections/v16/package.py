"""Build and validate a closed, package-rooted V16 development artifact set."""
...
from morocco_elections.v16.build import build_development_database
from morocco_elections.v16.publication import (
    PublicationContext,
    _claims,
    _evidence_bundle,
    _privacy,
...
from morocco_elections.v16.portability import portability_descriptor_sha256, validate_source_portability


DATABASE_NAME = "morocco_elections_v16.duckdb"
...
GEO_REPORT_NAME = "v16-geo-parent-report.json"
RECONCILIATION_REPORT_NAME = "v16-reconciliation-report.json"
RESULT_HISTORY_REPORT_NAME = "v16-result-history-report.json"
PACKAGE_FILES = (
    DATABASE_NAME,
    CONTRACT_NAME,
    V15_MANIFEST_NAME,
...
def _source_portability(repository_root: Path) -> list[dict[str, Any]]:
    ...
    official = json.loads(
        (repository_root / "metadata/v16/official_source_registry.json").read_text(encoding="utf-8")
    )
...
def _publication_reviews(
    ...
    for file in files:
        ...
        license_reviews.append({
            ...
            "reviewed_by": "V16_RELEASE_STEWARD",
        ...
        privacy_reviews.append({
            ...
            "file_sha256": digest,
            "decision": "PASS",
            "review_method": "V16_SCHEMA_AND_CONTENT_PRIVACY_REVIEW_V1",
            "reviewed_by": "V16_RELEASE_STEWARD",
            "reviewed_at": reviewed_at,
        ...
        claim_reviews.append({
            ...
            "review_method": "V16_CLAIM_CLASS_REVIEW_V1",
            "reviewed_by": "V16_RELEASE_STEWARD",
            "reviewed_at": reviewed_at,
            "evidence_id": evidence_id,
        })
...
def package_context(root: Path) -> PublicationContext:
    ...
    return PublicationContext(
        release_id=bundle.get("release_id", "V16-DEVELOPMENT"),
...
def build_package(
    ...
    *,
    ...
    if package_root.exists() and not replace_existing:
        raise FileExistsError(f"refusing to overwrite existing V16 package: {package_root}")
    ...
    try:
        database_report = build_development_database(
            ...
            legal_seed_path=repository_root / "metadata/v16/legal_regimes.seed.json",
            source_registry_path=repository_root / "metadata/v16/official_source_registry.json",
            evidence_root=repository_root,
            as_of_date=produced_at,
        )
        shutil.copy2(repository_root / "metadata/v16/data_contract.json", staging / CONTRACT_NAME)
        shutil.copy2(repository_root / "metadata/v16/v15_immutable_checksums.json", staging / V15_MANIFEST_NAME)
        ...
        context, _ = build_readiness_context(
            ...
            release_id="V16-DEVELOPMENT", as_of_date=produced_at,
            package_manifest_path=MANIFEST_NAME,
        ...
        if staged_validation["status"] != "PASS":
            raise RuntimeError("staged V16 package failed validation: " + json.dumps(staged_validation["failures"]))
        if backup.exists():
        ...
        if final_validation["status"] != "PASS":
            raise RuntimeError("moved V16 package failed validation")
        build_report = {
            **database_report,
