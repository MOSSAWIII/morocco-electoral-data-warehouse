...
from morocco_elections.warehouse.publication import sha256_file
...
def validate_legal_regime_seed(payload: Mapping[str, Any], source_registry: Mapping[str, Any]) -> list[ValidationIssue]:
    ...
    if payload.get("coverage_status") != "PARTIAL" or not payload.get("coverage_limitation"):
        issues.append(ValidationIssue("LEGAL_SEED_COVERAGE_NOT_DISCLOSED", "legal_regime_seed", "WAREHOUSE", "partial seed coverage and its limitation must be explicit"))
    return issues
