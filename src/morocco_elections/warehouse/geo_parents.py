"""Canonical parent relations without rewriting immutable snapshot geography rows."""
...
COMM2015_SEED_PATH = Path("metadata/warehouse/comm2015_geo_parents.seed.json")
...
def derive_historical_parent_relations(
    ...
    relations = [
        {
            "relation_id": f"WAREHOUSE-PARENT:{row['election_id']}:{row['geo_id']}",
...
def derive_comm2015_parent_relations(
    ...
    for row in arrondissement_rows:
        relations.append({
            "relation_id": f"WAREHOUSE-PARENT:COMM2015:{row['child_geo_id']}",
    ...
    for group in groups:
        ...
        relations.extend([
            {
                "relation_id": f"WAREHOUSE-PARENT:COMM2015:{group['parent_geo_id']}",
                ...
                "review_status": "VERIFIED_OFFICIAL_CROSSWALK",
                "supporting_source_ids": json.dumps([seed["code_source_id"], seed["legal_source_id"]], separators=(",", ":")),
                "notes": "Internal warehouse commune-parent identifier; not asserted as an official identifier.",
            },
            {
                "relation_id": f"WAREHOUSE-PARENT:COMM2015:{group['prefecture_geo_id']}",
...
def build_geo_parent_evidence(connection: duckdb.DuckDBPyConnection, root: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if issues:
        raise ValueError("invalid canonical geo parent evidence: " + ", ".join(sorted({issue.code for issue in issues})))
