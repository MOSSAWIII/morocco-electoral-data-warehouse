...
def _load_candidates(root: Path, bo_source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(rows) != 1206:
        raise ValueError(f"expected 1206 BO candidate rows, observed {len(rows)}")
    return rows
...
def build_reconciliation(root: Path) -> dict[str, Any]:
    ...
    return {"schema_version": 1, "scope": "BO6374_PAGES_6105_6129_CANDIDATE_ONLY", "sources": descriptors, "rows": rows, "report": report}
