...
from morocco_elections.warehouse.build import build_development_database  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build additive warehouse development database from immutable V15")
    parser.add_argument("--seed", type=Path, default=ROOT / "data/exports/open/v15/morocco_elections_v15.duckdb")
    parser.add_argument("--output", type=Path, required=True, help="warehouse development database path")
    parser.add_argument("--replace", action="store_true", help="atomically replace an existing warehouse output")
