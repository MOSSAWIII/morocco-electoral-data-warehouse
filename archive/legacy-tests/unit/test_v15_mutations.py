...
def test_production_relationship_validator_detects_orphan_party(tmp_path: Path) -> None:
    ...
    contracts = {
        name: next(row for row in all_table_contracts() if row["table_name"] == name)
        for name in ("dim_party", "fact_election_result")
    }
    connection.execute(_ddl(contracts["dim_party"], include_foreign_keys=False))
    connection.execute(_ddl(contracts["fact_election_result"], include_foreign_keys=False))
    connection.execute(
        "INSERT INTO fact_election_result"
        "(result_id,contest_id,election_id,geo_id,party_id,votes,source_id,evidence_id,source_row,"
        "identity_review_status,quality_status) "
        "VALUES ('R1','C1','E1','G1','MISSING',10,'S1','EV1',1,'VERIFIED','OK')"
    )
    connection.close()
    manifest = {
        "tables": list(contracts.values()),
        "relationships": [{
            "child_table": "fact_election_result", "child_column": "party_id",
            "parent_table": "dim_party", "parent_column": "party_id",
        }],
    }
